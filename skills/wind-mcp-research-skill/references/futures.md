...
> 本文件由 `scripts/registry.json` 生成（vserver_futures_data，7 个工具 / 22 个参数），不要手改。
...
| --- | --- | --- | --- | --- |
| `futures_get_warehouse_receipt` | 按业务类型、期货品种代码及日期查询期货品种的交割或仓单汇总，支持单品种、多品种和全市场。 | 交割数据按月、仓单数据按日 | windCodes, date, type | `{"type":"receipt","windCodes":["CU.SHF","AL.SHF"],"date":"2026-09-03"}` |
...
| `futures_get_position_ranking` | 按品种和交易日查询交易所公开席位的九类排名，涵盖持仓、增减仓和成交量排名。 | 仅反映交易所公开席位排名，不等同客户持仓归因或交易策略 | **windCode**, date, limit, type | `{"type":1,"windCode":"CU.SHF","date":"2026-09-03","limit":5}` |
| `futures_get_research_opinion` | 按单个期货品种和日期汇总公开研报的方向分类、统计结果、核心摘要及近 30 个交易日变化。 | 仅支持品种级查询，不替代研报原文核验、行情查询或供需数据 | **windCode**, date | `{"windCode":"CU.SHF"}` |
| `futures_get_supply_demand` | 按期货品种和基本面类型查询商品供需指标的最新值与历史时间序列，可按日期窗口和历史开关控制范围。 | 指标范围由已配置的 EDB 数据覆盖 | **windCode**, startDate, endDate, includeHistory, type | `{"windCode":"CU.SHF","type":4,"startDate":"2026-01-01","endDate":"2026-09-03"}` |
