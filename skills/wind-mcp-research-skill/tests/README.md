...
node tests/run-smoke.mjs                  # 全量冒烟：用注册表里的实测样例逐个调 134 个工具
...
| `run-smoke.mjs` | 134 个工具逐个真实调用 | 改 `annotations.json` 的样例之后，或怀疑后端有变时 |
...
之前把全量参数表写进目录，`company.md` 涨到 3.5 万字，而 agent 一次只用其中一个工具的参数。现在测试卡了两条线：单份 ≤ 1 万字、9 份合计 ≤ 4.2 万字，并且禁止目录里出现 `| 参数 | 必填 |` 表头。**「顺手把文档补全一点」会让这条测试红**，这是故意的。

## 后端变了怎么办

```bash
node scripts/cli.mjs doctor          # 看 Key、9 个 server 连通性、注册表漂移、上次自更新状态
...
`refresh` 只覆盖 schema，**不会动 `scripts/annotations.json`** 里的人工内容（server 说明、领域关键词、样例入参、已知故障、`boundary`、`extraParams`）。样例因 schema 变动而失效时，`run-offline-tests.mjs` 的「样例本身能通过校验」一项会点名到具体工具。

`extraParams` 是给「线上 schema 掉了、后端实测仍然接受」的参数留的口子（如 futures 三个工具的 `type`）：写在 annotations 里，refresh 时并回 `inputSchema.properties`，`describe` 会标 `backend_only`。线上 schema 把它加回来时，以线上为准，annotations 里的定义自动让位。
